...
public class UpdateMe {
    public static void UpdateMe(Message message){
        ...
        user.setCitizenURL(user.getCitizenURL().replaceAll("!updateme", "")); //removes the !newuser part of the string
        ...
        if(user.getCitizenURL().equals("")){
            ...
            //this verifies that the user has placed their DiscordID in their bio
            List<String> userPage = ScraperServices.checkPageUpdateMe(user.getCitizenURL(), user.getMessage(), driver, wait); //inputs a String and Message, outputs a string list
            ...
            if(userPage.size()==0){
                ...
                //set the user uee number
                user.setUeeCitizenRecord(userPage.get(1));

                //TODO remove any instance of the user in the database
                Bot.sendMessage(user.getMessage(),SQLServices.removeUser(user.getUeeCitizenRecord()));
