...
public class ScraperServices {
    ...
    public static ProfileClass scrapeBattleRoyal(ProfileClass player, WebDriver driver, WebDriverWait wait){
        try{
            ...
            //look for the user
            findPlayer.sendKeys(player.getHandle());
            ...
            try{
                ...
                MessageServices.sendMessage(player.getMessage(), ":small_orange_diamond: Battle Royal Leaderboard: Player not found.");
                player.setDiscordID(player.getMessage().getUserData().id().toString());
            ...
            //update user
            MessageServices.sendMessage(player.getMessage(), ":small_blue_diamond: Battle Royal Leaderboard Verified...");

            //return the results as an array. There are 16 results total
            return player;
        }catch(Exception e){
            MessageServices.sendMessage(player.getMessage(), ":small_red_triangle: Battle Royal Leaderboard Error: " + e);
            player.setBR_ELO(EloManagerServices.calculateInitialElo());
            player.setBR_Playtime(0);
            player.setBR_DamageDealt(0);
            player.setBR_DamageTaken(0);
    ...
    //check profile page
    public static ProfileClass checkPageNewUser(ProfileClass player, WebDriver driver, WebDriverWait wait) {
        ...
        try{
            ...
            if(driver.getPageSource().contains("You are currently venturing unknown space")){
                isRealUser = false;
            }else{
                isRealUser = true;
            }
            ...
            //if everything checks out, our player is real and verified. So we start scraping
            if(validURL==true && isRealUser==true && hasNumber==true) {
                ...
                if (SQLServices.existsUEENumber(player.getUeeCitizenRecord(), player.getMessage()) == true) {
                    return player;
                } else {//if the citizenrecord doesn't exist, finish scraping and return the elements in an array
                    //get their handle
                    player.setHandle(driver.findElement(By.cssSelector("#public-profile > div.profile-content.overview-content.clearfix > div.box-content.profile-wrapper.clearfix > div > div.profile.left-col > div > div.info > p:nth-child(2) > strong")).getText());

                    //get their Org SID
                    player.setOrgID(driver.findElement(By.cssSelector("#public-profile > div.profile-content.overview-content.clearfix > div.box-content.profile-wrapper.clearfix > div > div.main-org.right-col.visibility-V > div > div.info > p:nth-child(2) > strong")).getText());

                    //update user and close driver
                    MessageServices.sendMessage(player.getMessage(), ":small_blue_diamond: Profile Page verified...");
                    return player;
                }
